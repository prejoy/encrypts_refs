...
/*
void sm3_context_copy(sm3_context *ctx,sm3_context *ctx0)
{
	memcpy(ctx,ctx0,sizeof(sm3_context));
}
*/
                      ...
                      unsigned int keylen, unsigned char *out)
{
    ...
    sm3_hmac_starts( &ctx0, (unsigned char*)pass, passlen);

    sm3_starts( &ctx1 );
    sm3_update( &ctx1, ctx0.opad, 64 );

    while (keylen) 
	{
		cplen = (keylen > 32) ? 32 : keylen;
        /*
		...
		memcpy(&ctx,&ctx0,sizeof(sm3_context));
	    sm3_update( &ctx, (unsigned char*)salt, saltlen);
		sm3_update( &ctx, itmp, 4);
		sm3_finish( &ctx, digtmp );

		memcpy(&ctx,&ctx1,sizeof(sm3_context));
		sm3_update( &ctx, digtmp, 32);
		sm3_finish( &ctx, digtmp );
	...
	memset( &ctx0, 0, sizeof( sm3_context ) );
    memset( &ctx1, 0, sizeof( sm3_context ) );
	memset( &ctx, 0, sizeof( sm3_context ) );
	memset( digtmp, 0, sizeof( digtmp ) );
